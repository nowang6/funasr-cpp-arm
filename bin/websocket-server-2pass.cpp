...
nlohmann::json handle_result(FUNASR_RESULT result, bool is_final, const std::string& mode, const std::string& wav_name, std::string& accumulated_text) {
  ...
  if (tmp_online_msg != "") {
    ...
    text = tmp_online_msg;
    result_mode = "2pass-online";
  }
  std::string tmp_tpass_msg = FunASRGetTpassResult(result, 0);
  if (tmp_tpass_msg != "") {
    LOG(INFO) << "offline results : " << tmp_tpass_msg;
    text = tmp_tpass_msg;
    result_mode = "2pass-offline";
  }

  // 如果当次模型推理结果为空
  // - 如果是最终响应(is_final=true)，仍然构建完整的响应格式，文本返回空
  // - 如果不是最终响应，直接返回空json，不作响应
  if (text.empty() && !is_final) {
    LOG(INFO) << "current inference result is empty and not final, skip response";
    return jsonresult;  // 返回空json
  }

  // 文本累加逻辑：以mode包含offline为标志，两次offline之间，响应的文本内容是累加的
  // - online时：累加文本，响应累加后的文本
  // - offline时：响应当前文本（offline的文本已经是完整的），然后重置累加器
  std::string response_text = "";
  bool is_offline = (mode.find("offline") != std::string::npos || result_mode.find("offline") != std::string::npos);
  
  if (is_offline) {
    // offline模式：返回当前文本（不累加），然后重置累加器
    response_text = text;
    accumulated_text = "";  // 重置累加器，为下一个区间做准备
    LOG(INFO) << "offline mode, response_text: " << response_text << ", reset accumulated_text";
  } else {
    // online模式：累加文本，响应累加后的文本
    accumulated_text += text;
    response_text = accumulated_text;
    LOG(INFO) << "online mode, accumulated_text: " << accumulated_text << ", response_text: " << response_text;
  }

  // 构建新格式的响应
  // header 部分
  jsonresult["header"]["traceId"] = wav_name;
  jsonresult["header"]["status"] = is_final ? 2 : 1;
  
  // 确定 msgtype：
  // - 如果是最终响应(is_final=true)，设置为 "sentence"
  // - 如果 mode 或 result_mode 包含 "offline"，设置为 "sentence"
  // - 否则为 "progressive"
  std::string msgtype = "progressive";
  if (is_final || is_offline) {
    msgtype = "sentence";
  }
  
  // payload 部分
  jsonresult["payload"]["result"]["msgtype"] = msgtype;
  
  // 构建 ws 数组
  nlohmann::json ws_array = nlohmann::json::array();
  nlohmann::json ws_item;
  nlohmann::json cw_array = nlohmann::json::array();
  nlohmann::json cw_item;
  cw_item["w"] = response_text;
  cw_item["rl"] = "0";
  cw_array.push_back(cw_item);
  ws_item["cw"] = cw_array;
  ws_array.push_back(ws_item);
  jsonresult["payload"]["result"]["ws"] = ws_array;
...
void WebSocketServer::do_decoder(
  ...
  try {
    ...
    while (buffer.size() >= 800 * 2 && !msg["is_eof"]) {
      ...
      if (Result) {
        websocketpp::lib::error_code ec;
        nlohmann::json jsonresult = handle_result(Result, false, modetype, wav_name, accumulated_text);
        // 只有当handle_result返回非空json且识别结果不为空时才发送
        if (!jsonresult.empty() && jsonresult["payload"]["result"]["ws"][0]["cw"][0]["w"] != "") {
          std::string response_str = jsonresult.dump();
          LOG(INFO) << "[发送响应-在线] wav_name=" << wav_name << ", response=" << response_str;
          if (is_ssl) {
            wss_server_->send(hdl, response_str,
                              websocketpp::frame::opcode::text, ec);
          } else {
            server_->send(hdl, response_str,
                          websocketpp::frame::opcode::text, ec);
          }
        }
    ...
    if (is_final && !msg["is_eof"]) {
      ...
      if (Result) {
        websocketpp::lib::error_code ec;
        nlohmann::json jsonresult = handle_result(Result, true, modetype, wav_name, accumulated_text);
        std::string response_str = jsonresult.dump();
        LOG(INFO) << "[发送响应-最终] wav_name=" << wav_name << ", response=" << response_str;
        if (is_ssl) {
          wss_server_->send(hdl, response_str,
                            websocketpp::frame::opcode::text, ec);
        } else {
        ...
        if(wav_format != "pcm" && wav_format != "PCM"){
          ...
          // 构建错误消息的新格式
          jsonresult["header"]["traceId"] = wav_name;
          jsonresult["header"]["status"] = 2;
          jsonresult["payload"]["result"]["msgtype"] = "sentence";
          nlohmann::json ws_array = nlohmann::json::array();
          nlohmann::json ws_item;
          nlohmann::json cw_array = nlohmann::json::array();
          nlohmann::json cw_item;
          cw_item["w"] = "ERROR. Real-time transcription service ONLY SUPPORT PCM stream.";
          cw_item["rl"] = "0";
          cw_array.push_back(cw_item);
          ws_item["cw"] = cw_array;
          ws_array.push_back(ws_item);
          jsonresult["payload"]["result"]["ws"] = ws_array;
          std::string response_str = jsonresult.dump();
          LOG(INFO) << "[发送响应-错误] wav_name=" << wav_name << ", response=" << response_str;
          if (is_ssl) {
            wss_server_->send(hdl, response_str,
                              websocketpp::frame::opcode::text, ec);
          } else {
  ...
  switch (msg->get_opcode()) {
    case websocketpp::frame::opcode::text: {
      ...
      if (jsonresult.contains("chunk_size")) {
        ...
        // 如果没有收到 chunk_size，使用默认值初始化 tpass_online_handle
        if (msg_data->tpass_online_handle == nullptr) {
          // 默认 chunk_size: [5, 10, 5] (常见的默认值)
          std::vector<int> default_chunk_size = {5, 10, 5};
          LOG(INFO) << "No chunk_size provided, using default: [5, 10, 5]";
          FUNASR_HANDLE tpass_online_handle =
              FunTpassOnlineInit(tpass_handle, default_chunk_size);
          msg_data->tpass_online_handle = tpass_online_handle;
        }
      }
      ...
      // 处理文本消息中的base64编码音频数据 (适配Java客户端)
      bool base64_final_processed = false;  // 标记base64处理块是否已处理最终数据
      if (jsonresult.contains("payload") && jsonresult["payload"].contains("audio") && 
          jsonresult["payload"]["audio"].contains("audio")) {
        try {
          std::string audio_b64 = jsonresult["payload"]["audio"]["audio"].get<std::string>();
          if (!audio_b64.empty()) {
            // 解码base64音频数据
            std::string audio_data = websocketpp::base64_decode(audio_b64);
            if (!audio_data.empty()) {
              // 将解码后的音频数据添加到sample_data_p
              const char* pcm_data = audio_data.data();
              int32_t num_samples = audio_data.size();
              sample_data_p->insert(sample_data_p->end(), pcm_data, pcm_data + num_samples);
              
              // 检查是否为最后一帧 (status == 2)
              bool is_final_frame = false;
              if (jsonresult.contains("header") && jsonresult["header"].contains("status")) {
                int status = jsonresult["header"]["status"].get<int>();
                if (status == 2) {
                  is_final_frame = true;
                  LOG(INFO) << "Received final frame (status=2) in base64 message";
                }
              }
              
              // 如果是2pass模式且在线处理，需要实时处理音频数据
              std::string mode_str = msg_data->msg["mode"].get<std::string>();
              if (isonline && mode_str == "2pass") {
                int setpsize = 800 * 2;  // 步长
                // 如果sample_data大小超过步长，提交数据解码
                if (sample_data_p->size() > setpsize) {
                  int chunksize = floor(sample_data_p->size() / setpsize);
                  // 确保子向量大小是setpsize的整数倍
                  ...
                  try {
                    ...
                    if (msg_data->msg["is_eof"] != true && msg_data->hotwords_embedding != nullptr) {
                      ...
                      // 使用hdl的副本而非std::move，因为后续可能还需要使用hdl（如最终帧处理）
                      websocketpp::connection_hdl hdl_copy = hdl;
                      msg_data->strand_->post(
                                std::bind(&WebSocketServer::do_decoder, this,
                                          std::move(subvector), std::move(hdl_copy),
                                          ...
                                          std::ref(msg_data->accumulated_text)));
                ...
                // 如果是最后一帧，处理剩余数据
                if (is_final_frame && msg_data->msg["is_eof"] != true && 
                    msg_data->hotwords_embedding != nullptr) {
                  try {
                    // 创建剩余数据的副本用于处理
                    std::vector<char> remaining_data = *(sample_data_p.get());
                    sample_data_p->clear();  // 清空，避免重复处理
                    
                    std::vector<std::vector<float>> hotwords_embedding_(*(msg_data->hotwords_embedding));
                    msg_data->strand_->post(
                        std::bind(&WebSocketServer::do_decoder, this,
                                  std::move(remaining_data), std::move(hdl),
                                  ...
                                  std::ref(msg_data->accumulated_text)));
                    msg_data->msg["access_num"]=(int)(msg_data->msg["access_num"])+1;
                    base64_final_processed = true;  // 标记已处理
                    LOG(INFO) << "Base64 final frame processed in online 2pass mode";
                  }
                  catch (std::exception const &e)
                  {
                    LOG(ERROR)<<e.what();
                ...
                // 如果不是在线2pass模式，但收到status=2，标记需要后续处理
                // 这种情况会在下面的最终处理逻辑中处理
                if (is_final_frame) {
                  LOG(INFO) << "Base64 final frame (status=2) received, will be processed in final handler";
                }
              }
            }
          }
        } catch (std::exception const &e) {
          LOG(ERROR) << "Failed to decode base64 audio data: " << e.what();
        }
      }
      ...
      // 检查status=2（最后一帧）或传统的is_speaking/is_finished标志
      // 如果base64处理块已经处理了最终数据，则跳过
      bool is_final_message = false;
      if (!base64_final_processed) {
        if (jsonresult.contains("header") && jsonresult["header"].contains("status")) {
          int status = jsonresult["header"]["status"].get<int>();
          if (status == 2) {
            is_final_message = true;
            LOG(INFO) << "Received final message (status=2)";
          }
        }
        if (!is_final_message && jsonresult.contains("is_speaking")) {
          is_final_message = (jsonresult["is_speaking"] == false);
        }
        if (!is_final_message && jsonresult.contains("is_finished")) {
          is_final_message = (jsonresult["is_finished"] == true);
        }
      }
      ...
      if (is_final_message && 
          msg_data->msg["is_eof"] != true &&
          msg_data->hotwords_embedding != nullptr &&
          sample_data_p->size() > 0) {  // 确保有数据需要处理
